...
locals {
  internet_gateway_exists = data.aws_internet_gateway.existing_gw.id != "" ? true : false
}

# Conditional creation of the internet gateway if one doesn't already exist
resource "aws_internet_gateway" "stage-gw" {
  count = local.internet_gateway_exists ? 0 : 1  # Only create if no existing internet gateway
